...
listKeywordsCredits = ["Kredit", "Krediter", "Credit", "Universitet", "Kurs", "Course", "hp ", "poäng",
...
def fileCategorizedInMap(directory_Inputfiles):
    ...
    for filename in listdir(directory_Inputfiles):
        ...
        if (totalOccurrencesPayment > totalOccurrencesCredits) and (
                totalOccurrencesPayment > totalOccurrencesRepayment) and (
                totalOccurrencesPayment > totalOccurrencesProcessingTime) and (
                totalOccurrencesPayment > totalOccurrencesStudyAssurance):
            try:
                dict_obj["Studiemedel"].append(filename)
            except KeyError:
                dict_obj["Studiemedel"] = [filename]
        elif (totalOccurrencesCredits > totalOccurrencesPayment) and (
                totalOccurrencesCredits > totalOccurrencesRepayment) and (
                totalOccurrencesCredits > totalOccurrencesProcessingTime) and (
                totalOccurrencesCredits > totalOccurrencesStudyAssurance):
            try:
                dict_obj["Studieresultat"].append(filename)
            except KeyError:
                dict_obj["Studieresultat"] = [filename]
        elif (totalOccurrencesRepayment > totalOccurrencesCredits) and (
                totalOccurrencesRepayment > totalOccurrencesPayment) and (
                totalOccurrencesRepayment > totalOccurrencesProcessingTime) and (
                totalOccurrencesRepayment > totalOccurrencesStudyAssurance):
            try:
                dict_obj["Återbetalning"].append(filename)
            except KeyError:
                dict_obj["Återbetalning"] = [filename]
        elif (totalOccurrencesProcessingTime > totalOccurrencesCredits) and (
                totalOccurrencesProcessingTime > totalOccurrencesRepayment) and (
                totalOccurrencesProcessingTime > totalOccurrencesPayment) and (
                totalOccurrencesProcessingTime > totalOccurrencesStudyAssurance):
            try:
                dict_obj["Väntetid"].append(filename)
            except KeyError:
                dict_obj["Väntetid"] = [filename]
        elif (totalOccurrencesStudyAssurance > totalOccurrencesCredits) and (
                totalOccurrencesStudyAssurance > totalOccurrencesRepayment) and (
                totalOccurrencesStudyAssurance > totalOccurrencesProcessingTime) and (
                totalOccurrencesStudyAssurance > totalOccurrencesPayment):
            try:
...
# _________________RESULT.Output IN TERMINAL: displays the dictionary with categories and respective files in the terminal //TW 07.10.2019_________________________
dict_obj = fileCategorizedInMap(directory_Inputfiles)
print("\n")
for key, val in dict_obj.items():
    print(key, "|", val)
#_________________RESULT.Output IN NEW FILE "table.txt": displays the dictionary with the categories and respective files in a new file // TW.07.10.2019
